...
export const Easing = {
  ...
  cubicBezier: (x1, y1, x2, y2) => {
    ...
    return (t) => {
      ...
      for (let i = 0; i < 8; i++) {
        const z = x * x * x * (1 - x1) + 3 * x * x * (1 - x) * x1 + 3 * x * (1 - x) * (1 - x) * 0 + (1 - x) * (1 - x) * (1 - x) * 0 - t;
        if (Math.abs(z) < epsilon) break;
        const dz = 3 * x * x * (x2 - x1) + 6 * x * (1 - x) * (x1 - 0) + 3 * (1 - x) * (1 - x) * (0 - 0);
        if (Math.abs(dz) < epsilon) break;
        x = x - z / dz;
      }
      
      return x * x * x * (1 - y2) + 3 * x * x * (1 - x) * y2 + 3 * x * (1 - x) * (1 - x) * y1 + (1 - x) * (1 - x) * (1 - x) * 0;
    };
  },
};
