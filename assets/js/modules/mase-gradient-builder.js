	...
	MASE.gradientBuilder = {
		...
		initTypeSelector: function() {
			...
			$('.mase-gradient-type').on('change', function() {
				...
				const $angleRow = $select.closest('.mase-background-gradient-controls').find('.mase-gradient-angle-row');
		...
		initAngleControl: function() {
			...
			$('.mase-gradient-angle-dial').on('mousedown', function(e) {
				...
				const $input = $dial.closest('.mase-gradient-angle-control').find('.mase-gradient-angle-input');
			...
			$('.mase-gradient-angle-input').on('input change', function() {
				...
				const $dial = $input.closest('.mase-gradient-angle-control').find('.mase-gradient-angle-dial');
		...
		addColorStop: function(area) {
			...
			const $newStop = $('<div class="mase-color-stop" data-index="' + index + '">' +
				'<div class="mase-color-stop-color">' +
					'<input type="text" ' +
						'name="custom_backgrounds[' + area + '][gradient_colors][' + index + '][color]" ' +
		...
		reindexColorStops: function(area) {
			...
			$container.find('.mase-color-stop').each(function(index) {
				...
				// Update input names
				$stop.find('.mase-color-picker').attr('name', 'custom_backgrounds[' + area + '][gradient_colors][' + index + '][color]');
				$stop.find('.mase-stop-position').attr('name', 'custom_backgrounds[' + area + '][gradient_colors][' + index + '][position]');
				
				// Update aria labels
				$stop.find('.mase-color-picker').attr('aria-label', 'Color stop ' + (index + 1) + ' color');
				$stop.find('.mase-stop-position').attr('aria-label', 'Color stop ' + (index + 1) + ' position');
				$stop.find('.mase-remove-color-stop').attr('aria-label', 'Remove color stop ' + (index + 1));
			});
		},
		...
		getGradientConfig: function(area) {
			const $controls = $('.mase-background-gradient-controls[data-type="gradient"]').filter(function() {
		...
		applyPreset: function(area, category, presetId) {
			...
			$grid.find('.mase-gradient-preset-item[data-preset-id="' + presetId + '"]').addClass('selected');
