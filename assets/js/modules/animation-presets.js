...
class AnimationPresetsLibrary {
  ...
  _initializeBuiltInPresets() {
    // Fade animations
    this.addPreset(new AnimationPreset(
      'fade-in',
      'Fade In',
    ...
    this.addPreset(new AnimationPreset(
      ...
      { duration: 400, easing: 'cubic-bezier(0.175, 0.885, 0.32, 1.275)', tags: ['entrance', 'zoom', 'bounce'] }
    ));
  ...
  _createFilters() {
    ...
    categories.forEach(category => {
      ...
      btn.onclick = () => {
        ...
        filters.querySelectorAll('.mase-preset-filter-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        this._renderPresets();
      };
