...
class PreviewEngine {
  ...
  generateCSS(settings, forceFullGeneration = false) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        if (duration > 50) {
          console.warn(
            `MASE Preview Engine: CSS generation exceeded 50ms target (${duration.toFixed(2)}ms)`,
          );
        }
      }
  ...
  generateIncrementalCSS(settings) {
    ...
    const adminBarChanged =
      JSON.stringify(settings.admin_bar) !== JSON.stringify(this.previousSettings.admin_bar);
    const adminMenuChanged =
      JSON.stringify(settings.admin_menu) !== JSON.stringify(this.previousSettings.admin_menu);
    const typographyChanged =
      JSON.stringify(settings.typography) !== JSON.stringify(this.previousSettings.typography);
    const visualEffectsChanged =
      JSON.stringify(settings.visual_effects) !==
