    ...
    addAriaLabels: function () {
      ...
      $('.mase-background-type-selector').each(function () {
        ...
        if (!$select.attr('aria-label') && !$select.attr('aria-labelledby')) {
          ...
          const areaLabel = $select
            .closest('.mase-background-area-section')
            .find('.mase-background-area-title h3')
            .text()
            .trim();
      ...
      $('.mase-background-upload-zone').each(function () {
        ...
        if (!$zone.attr('aria-label')) {
          $zone.attr(
            'aria-label',
            'Upload or select background image. Click to open media library or drag and drop an image file.',
          );
        }
      ...
      $('.mase-gradient-angle-dial').each(function () {
        if (!$(this).attr('aria-label')) {
          $(this).attr(
            'aria-label',
            'Gradient angle visual indicator. Use the numeric input to change the angle.',
          );
        }
        if (!$(this).attr('role')) {
          $(this).attr('role', 'img');
