...
(function ($) {
  ...
  MASE.compatibility = {
    ...
    detectFeatures: function () {
      ...
      // Drag and Drop API
      this.features.dragDrop =
        'draggable' in document.createElement('div') && 'ondrop' in document.createElement('div');
    ...
    detectBlendModes: function () {
      ...
      return (
        CSS.supports('mix-blend-mode', 'overlay') || CSS.supports('mix-blend-mode', 'multiply')
      );
    },
    ...
    detectGradients: function () {
      ...
      return (
        CSS.supports('background-image', 'linear-gradient(#fff, #000)') ||
    ...
    provideLazyLoadFallback: function () {
      ...
      $(document).on('mase:background-image-set', function (e, area, imageUrl) {
        const selector = MASE.compatibility.getAreaSelector(area);
      ...
      if (this.isOutdatedBrowser()) {
        ...
        const $notice = $(
          '<div class="notice notice-error mase-outdated-browser-notice">' +
            '<p><strong>Outdated Browser:</strong> ' +
            message +
            '</p>' +
            '</div>',
        );
